...
def main():
    ...
    try:
        ...
        print("   4️⃣  Очікування healthy статусу всіх контейнерів")
        if mode == 'local':
            ...
            print("   8️⃣  Генерація тестових оголошень (мінімум 10)")
        print("   9️⃣  Виведення інформації з посиланнями")
        print()
        ...
        # ЕТАП 5: Перехід в каталог frontend та npm install
        if mode == 'local':
            print_step(5, "Встановлення залежностей frontend (npm install)")
        ...
        # ЕТАП 8: Генерація тестових оголошень (тільки для local mode)
        if mode == 'local' and frontend_ready:
            print_step(8, "Генерація тестових оголошень")
            try:
                print("🎭 Запуск генератора тестових оголошень...")
                print("   Мінімум 10 оголошень буде створено автоматично")
                
                # Запускаємо Node.js скрипт генератора
                generator_script = os.path.join(project_root, 'frontend', 'scripts', 'run-generator.js')
                ...
                if os.path.exists(generator_script):
                    ...
                    if generator_result.returncode == 0:
                        print_success("✅ Тестові оголошення створено успішно!")
                        # Виводимо останні рядки з результатом
                        output_lines = generator_result.stdout.strip().split('\n')
                        for line in output_lines[-10:]:
                            if line.strip():
                                print(f"   {line}")
                    else:
                        print_warning("⚠️ Генератор завершився з помилкою")
                        print(f"   {generator_result.stderr[:200]}")
                else:
                    print_warning(f"⚠️ Скрипт генератора не знайдено: {generator_script}")
                    print("   Ви можете запустити його вручну пізніше:")
                    print(f"   cd frontend && node scripts/run-generator.js")
                    
            except subprocess.TimeoutExpired:
                print_warning("⚠️ Генератор перевищив ліміт часу (5 хв)")
            except Exception as e:
                print_warning(f"⚠️ Помилка при запуску генератора: {e}")
                print("   Ви можете запустити його вручну:")
                print("   cd frontend && node scripts/run-generator.js")

        # ЕТАП 9: Виведення інформації з посиланнями
        print_step(9, "Виведення інформації з посиланнями")
