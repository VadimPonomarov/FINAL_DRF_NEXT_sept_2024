...
class CarAdImageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AddImageModel
        fields = ['id', 'image', 'image_url', 'image_display_url', 'order', 'is_primary', 'caption']
...
class CarAdSerializer(BaseModelSerializer):
    ...
    vehicle_type = serializers.IntegerField(source='mark.vehicle_type.id', read_only=True)
    vehicle_type_name = serializers.CharField(source='mark.vehicle_type.name', read_only=True)
    ...
    class Meta(BaseModelSerializer.Meta):
        model = CarAd
        fields = [
            'id', 'account', 'mark', 'model', 'title', 'description',
            'price', 'currency', 'dynamic_fields', 'region', 'city',
            'seller_type', 'exchange_status', 'is_validated',
        ...
        if title or description:
            ...
            try:
                ...
                if moderation_result.status.value == 'rejected':
                    ...
                    censored_title = self._apply_censorship(title or '', moderation_result.censored_text)
                    censored_description = self._apply_censorship(description or '', moderation_result.censored_text)
                ...
                if found_profanity:
                    raise serializers.ValidationError({
                        ...
                        'suggestions': ['Remove inappropriate language from title and description'],
    ...
    def to_representation(self, instance):
        ...
        data['moderation_info'] = {
            ...
            'moderated_by': instance.moderated_by.get_full_name() if instance.moderated_by else 'Auto-moderation',
        ...
        if instance.validation_errors:
            data['moderation_info'].update({
                'llm_status': instance.validation_errors.get('moderation_status', 'unknown'),
    ...
    def get_view_count(self, obj):
        ...
        try:
            ...
            owner_user = obj.account.user if getattr(obj, 'account', None) and getattr(obj.account, 'user', None) else None
    ...
    def _update_account_contacts(self, account, contacts_data):
        ...
        # Log update
        print(f"[CarAdSerializer] Updated {len(contacts_data)} contacts for account {account.id}")
    ...
    def get_phone_views_count(self, obj):
        ...
        try:
            ...
            users_count = qs.filter(user__isnull=False).values('user_id').distinct().count()
            anon_count = qs.filter(user__isnull=True).values('session_id').distinct().count()
