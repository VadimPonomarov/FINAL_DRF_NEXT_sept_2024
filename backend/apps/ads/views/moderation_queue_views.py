...
class ModerationQueueView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return CarAd.objects.filter(
            status__in=[AdStatusEnum.PENDING, AdStatusEnum.NEEDS_REVIEW, AdStatusEnum.REJECTED, AdStatusEnum.BLOCKED, AdStatusEnum.ACTIVE]
        ).select_related(
...
@swagger_auto_schema(
    ...
    request_body=openapi.Schema(
        ...
        properties={
            ...
            'moderator_notes': openapi.Schema(
                type=openapi.TYPE_STRING,
                description="Additional moderator notes"
            )
        },
        required=['reason']
    ),
    responses={
        200: openapi.Response(
            description="Advertisement blocked successfully",
            schema=CarAdSerializer
        ),
        403: openapi.Response(description="Permission denied"),
        404: openapi.Response(description="Advertisement not found")
...
@swagger_auto_schema(
    ...
    responses={
        200: openapi.Response(
            description="Advertisement marked for review",
            schema=CarAdSerializer
        ),
        403: openapi.Response(description="Permission denied"),
        404: openapi.Response(description="Advertisement not found")
...
def moderation_statistics(request):
    ...
    stats = {
        ...
        'today_moderated': CarAd.objects.filter(
