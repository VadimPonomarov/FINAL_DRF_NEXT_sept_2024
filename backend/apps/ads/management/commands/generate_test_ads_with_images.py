    ...
    def _regenerate_ads_without_real_images(self, created_ads, image_types):
        ...
        for ad in created_ads:
            ...
            for img in images_qs:
                ...
                # Detect placeholder / mock images that should NOT be treated as real photos
                is_placeholder = False

                # Old-style placeholder URLs with explicit marker
                if 'placeholder_' in url:
                    is_placeholder = True

                # ChatAI local mock images saved under generated-images/mock-*.jpg
                if 'generated-images' in url and 'mock-' in url:
                    is_placeholder = True

                # Data-URL SVG fallbacks generated by ChatAI mock implementation
                if url.startswith('data:image/svg+xml'):
                    is_placeholder = True

                # Images explicitly captioned as placeholders
                caption = (getattr(img, 'caption', '') or '').lower()
                if caption.startswith('placeholder'):
                    is_placeholder = True

                # Only count as real if none of the placeholder heuristics matched
        ...
        for ad in problematic_ads:
            try:
                ...
                # Re-check images after regeneration using the same placeholder heuristics
                images_qs_after = ad.images.all()
                has_real_after = False

                for img in images_qs_after:
                    url = ''
                    if img.image_url:
                        url = str(img.image_url)
                    elif getattr(img.image, 'url', None):
                    ...
                    # Detect placeholder / mock images
                    is_placeholder = False
                    if 'placeholder_' in url:
                        is_placeholder = True
                    if 'generated-images' in url and 'mock-' in url:
                ...
                if not has_real_after:
                    self.stdout.write(f'   🗑️ Deleting ad #{ad.id} with no real images after regeneration')
                    AddImageModel.objects.filter(ad=ad).delete()
                    ads_to_delete.append(ad)
                else:
                    self.stdout.write(f'   ✅ Regenerated real images for ad #{ad.id}')

            except Exception as e:
                self.stdout.write(f'   ❌ Error regenerating images for ad #{ad.id}: {e}')

        # Physically delete ads that still have no real images and remove them from created_ads list
        for ad in ads_to_delete:
            ad_id = ad.id
            ad.delete()
            if ad in created_ads:
                created_ads.remove(ad)
            self.stdout.write(f'   🗑️ Ad #{ad_id} removed from dataset due to missing real images')
