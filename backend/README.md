# 🚗 AutoRia - Интеллектуальная платформа автомобильных объявлений

## 📋 Описание проекта

AutoRia - это современная платформа для размещения автомобильных объявлений с интеллектуальной системой модерации на основе LLM и автоматическим обновлением курсов валют. Система построена на Django REST Framework с использованием микросервисной архитектуры.
...
- **Кэширование**: Redis
- **Модерация**: LLM-система с промптами
- **Валюты**: Автообновление через API НБУ/ПриватБанк
- **Тестирование**: Postman коллекции

## 🧠 Интеллектуальная система модерации

### Особенности LLM-модерации:

#### 🌍 **Мультиязычная модерация**
- **Украинский язык**: `блять`, `хуй`, `пизда`, `сука` → автоблокировка
- **Русский язык**: `блядь`, `хуй`, `пизда`, `сука` → автоблокировка  
- **Английский язык**: `fuck`, `shit`, `bitch`, `damn` → автоблокировка
- **Транслитерация**: `blyat`, `hui`, `pizda`, `suka` → автоблокировка
- **Замаскированная**: `bl4at`, `hu1`, `p1zda`, `s0ka` → интеллектуальное обнаружение

#### ⭐ **Цензурирование звездочками**
```
блять → б****
хуй → х**
пизда → п****
...
#### 🎯 **Двухэтапный процесс модерации**

**ЭТАП 1: LLM-анализ**
- Обнаружение нецензурной лексики на любом языке
- Тематическая проверка (только транспорт разрешен)
- Цензурирование найденных слов
- Определение языка и уверенности

**ЭТАП 2: Дополнительные проверки**
- Проверка адекватности цены
- Анализ длины контента
- Детекция спама (повторы, КАПС)
