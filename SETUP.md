...
Для сценарію «Docker для всіх сервісів, крім frontend, і локальний запуск frontend» оберіть:
- Спочатку введіть `2` — докеризація всіх контейнерів, крім frontend
- Потім введіть `0` — запуск фронтенда локально

Це підніме PostgreSQL, Redis, RabbitMQ, Backend, Celery у Docker, а фронтенд запуститься локально (`npm run start`).
