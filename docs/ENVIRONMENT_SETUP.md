...
Проект використовує модульну систему `.env` файлів у каталозі `env-config/` для підтримки різних конфігурацій розгортання. Основний Python-лоадер [`env-config/load-env.py`](../env-config/load-env.py) об'єднує `.env.base`, `.env.local`/`.env.docker` та `.env.secrets`, валідує значення й формує єдину конфігурацію середовища. Для фронтенду додатково використовуються скрипти `scripts/setup-frontend-env.py` та `scripts/setup-frontend-env.sh`, які генерують `frontend/.env.local` перед запуском або деплоєм.
