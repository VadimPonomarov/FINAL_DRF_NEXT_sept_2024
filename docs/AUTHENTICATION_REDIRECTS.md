...
2. **Рівень 2 — BackendTokenPresenceGate**  
   Перевіряє наявність backend токенів у httpOnly cookies та оновлює їх при потребі.  
...
graph TD
    ...
    E --> F[Backend login → httpOnly cookies]
    F --> A
...
| `frontend/src/app/api/auth/token/route.ts` | API для роботи з токенами в cookies |
...
- Токени зберігаються в httpOnly cookies, не в Redis.
...
**Статус**: Мігровано на httpOnly cookies + NextAuth
