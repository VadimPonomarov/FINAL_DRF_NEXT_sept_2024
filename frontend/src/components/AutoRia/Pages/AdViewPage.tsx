...
const AdViewPage: React.FC<AdViewPageProps> = ({ adId, showModerationControls = false }) => {
  ...
  const loadAdData = async () => {
    try {
      setIsLoading(true);
      setLoadError(null);
      console.log('[AdViewPage] Loading ad data for ID:', adId);
      ...
      console.log('[AdViewPage] Loaded ad data:', data);
      console.log('[AdViewPage] Images data:', data.images);
      console.log('[AdViewPage] Images count:', data.images?.length || 0);
      if (data.images && data.images.length > 0) {
        console.log('[AdViewPage] First image:', data.images[0]);
        console.log('[AdViewPage] First image display URL:', data.images[0].image_display_url);
      }
      ...
      if (user) {
        try {
          ...
          console.error('[AdViewPage] Error checking favorite status:', error);
          setIsFavorite(false);
        }
      } else {
        setIsFavorite(false);
      ...
      console.log('[AdViewPage] Counters loaded:', {
      ...
      console.error('[AdViewPage] Error loading ad:', error);
  ...
  useEffect(() => {
    ...
    const trackView = async () => {
      try {
        ...
        console.warn('[AdViewPage] view tracking failed', e);
      }
    };
    trackView();
  ...
  const handleToggleFavorite = useCallback(async () => {
    ...
    try {
      ...
      console.log('[AdViewPage] Favorite toggled:', {
        adId,
      ...
      console.error('[AdViewPage] Error toggling favorite:', error);
      toast({
        title: "Ошибка",
  ...
  console.log('[AdViewPage] reset button check:', {
                ...
                <div className="relative aspect-video bg-gray-100">
                  {adData.images && adData.images.length > 0 ? (
                    (() => {
                      ...
                      console.log('[AdViewPage] Rendering image:', { currentImageIndex, currentImage, imageUrl });
                      return (
                        <img
                          src={imageUrl}
                          alt={adData.title}
                          className="w-full h-full object-cover"
                          onLoad={() => console.log('[AdViewPage] Image loaded successfully:', imageUrl)}
                          onError={(e) => {
                            console.error('[AdViewPage] Image failed to load:', imageUrl);
