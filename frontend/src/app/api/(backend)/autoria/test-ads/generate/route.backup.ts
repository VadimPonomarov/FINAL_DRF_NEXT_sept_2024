...
const CREATE_AD_TIMEOUT_MS = 30000; // 30 seconds
const IMAGE_GEN_TIMEOUT_MS = 60000; // 60 seconds
const IMAGE_SAVE_TIMEOUT_MS = 15000; // 15 seconds
const BATCH_SIZE = 3;
const MAX_ADS_LIMIT = 50;

// Helper function to create authenticated fetch
function createAuthFetch(requestOrToken: NextRequest | string): AuthFetchFn {
  ...
  return (url, options = {}) => {
    ...
    const headers = mergeHeaders({ 'Content-Type': 'application/json' }, baseHeaders, options.headers);
    return fetch(url, { ...options, headers });
  };
}

// Helper function to create default authenticated fetch
function createDefaultAuthFetch(requestOrToken: NextRequest | string): AuthFetchFn {
  const accessToken = typeof requestOrToken === 'string' ? requestOrToken : null;
  const request = typeof requestOrToken === 'string' ? null : requestOrToken;
  ...
  return (url, options) => ServerAuthManager.authenticatedFetch(request!, url, options);
}
...
// Helper function to create a single ad with timeout and error handling
async function createSingleAd(
  authFetch: AuthFetchFn,
  defaultAuthFetch: AuthFetchFn,
  ...
  try {
    ...
    console.log(`🚗 [TestAds] Creating ad ${index + 1}:`, {
    ...
    // Create the ad
    const response = await authFetch(`${BACKEND_URL}/api/autoria/ads/`, {
      method: 'POST',
    ...
    if (!response.ok) {
      ...
      console.error(`❌ [TestAds] Failed to create ad ${index + 1}:`, {
    ...
    if (includeImages && imageTypes.length > 0) {
      const imageResult = await generateAndSaveImages(
        defaultAuthFetch,
        createdAd,
        imageTypes,
        index
    ...
    console.log(`✅ [TestAds] Ad ${index + 1} created successfully:`, {
      id: createdAd.id,
      title: formData.title,
    ...
    if (fetchError instanceof Error && fetchError.name === 'AbortError') {
      console.error(`⏱️ [TestAds] Request timeout creating ad ${index + 1} (${CREATE_AD_TIMEOUT_MS}ms)`);
      return {
        success: false,
        title: formData.title,
        imagesCount: 0,
    ...
    console.error(`❌ [TestAds] Error creating ad ${index + 1}:`, fetchError);
    return {
      success: false,
...
async function generateAndSaveImages(
  ...
  imageTypes: string[],
  adIndex: number
): Promise<{ savedCount: number; debugInfo: Record<string, unknown> }> {
  let savedCount = 0;
  let debugInfo: Record<string, unknown> = {};
  ...
  try {
    ...
    const { controller: genController, timeoutId: genTimeoutId } = createTimeoutController(IMAGE_GEN_TIMEOUT_MS);
    
    let genResp: Response;
    try {
      genResp = await defaultAuthFetch(`${BACKEND_URL}/api/chat/generate-car-images-mock/`, {
        method: 'POST',
        body: JSON.stringify({
          car_ad_id: createdAd.id,
      ...
      if (genError instanceof Error && genError.name === 'AbortError') {
        console.error(`⏱️ [TestAds] Image generation timeout for ad ${createdAd.id} (${IMAGE_GEN_TIMEOUT_MS}ms)`);
        return { savedCount: 0, debugInfo: { error: 'Image generation timeout' } };
      }
      throw genError;
    }
    ...
    try {
      ...
      console.error(`❌ [TestAds] Failed to parse image generation response:`, parseError);
      genData = { success: false, images: [] };
    }
    ...
    if (genData.success && Array.isArray(genData.images) && genData.images.length > 0) {
      const savePromises = genData.images.map(async (imageData: any, idx: number) => {
        try {
          ...
          try {
            const saveResp = await defaultAuthFetch(`${BACKEND_URL}/api/autoria/ads/${createdAd.id}/images/`, {
              method: 'POST',
              body: JSON.stringify({
                image_url: imageData.url,
                image_type: imageData.type || 'generated',
                alt_text: imageData.alt_text || `Generated image ${idx + 1}`
              }),
      ...
      savedCount = saveResults.filter(r => r.success).length;
      
      console.log(`💾 [TestAds] Saved ${savedCount}/${genData.images.length} images for ad ${createdAd.id}`);
    }
  } catch (error: any) {
    console.error(`❌ [TestAds] Error in image generation/saving for ad ${createdAd.id}:`, error);
    debugInfo = { error: error.message };
  }

  return { savedCount, debugInfo };
}
...
export async function createTestAdsServer(
  ...
  const defaultAuthFetch = createDefaultAuthFetch(requestOrToken);
  ...
  try {
    const modelsResponse = await defaultAuthFetch(`${BACKEND_URL}/api/public/reference/models?page_size=1000`);
  ...
  try {
    const usersResponse = await authFetch(`${BACKEND_URL}/api/autoria/users/`);
    if (usersResponse.ok) {
    ...
    try {
      ...
      if (includeImages) {
        try {
          ...
          // Add timeout for image generation (30 seconds - reduced for faster processing)
          const IMAGE_GEN_TIMEOUT_MS = 30000;
          const genController = new AbortController();
          const genTimeoutId = setTimeout(() => genController.abort(), IMAGE_GEN_TIMEOUT_MS);

          let genResp: Response;
          try {
            genResp = await fetch(`${backendUrl}/api/chat/generate-car-images-mock/`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                car_data: {
            ...
            if (fetchError instanceof Error && fetchError.name === 'AbortError') {
              ...
              genResp = new Response(JSON.stringify({
                success: false,
          ...
          if (genResp.ok || genResp.status === 408) {
            let genData: any;
            try {
              genData = await genResp.json();
            } catch (parseError) {
            ...
            if ((genData.success || genData.status === 'ok') && Array.isArray(genData.images)) {
              ...
              console.warn(`⚠️ [TestAds] Image generation returned no images or invalid response:`, {
                success: genData.success,
                status: genData.status,
                hasImages: Array.isArray(genData.images),
                imagesCount: genData.images?.length || 0
              });
              // Continue without images - don't fail the whole process
            }
          } else {
            const errorText = await genResp.text().catch(() => 'Unknown error');
            console.warn(`⚠️ [TestAds] Image generation failed (continuing without images):`, {
          ...
          console.warn(`⚠️ [TestAds] Image generation flow error (continuing without images):`, imgErr);
          // Continue without images - don't fail the whole process
        }
      }

      // Update progress
      const adProgress = Math.round(((i + 1) / count) * 100);
      onProgress?.(adProgress, `Создано объявление ${i + 1}/${count}${savedCount > 0 ? ` с ${savedCount} изображениями` : ''}`);
